...
document.addEventListener("DOMContentLoaded", function () {
  const editButtons = document.querySelectorAll(".link_edit_review");
  const modal = new bootstrap.Modal(document.getElementById("editReviewModal"));

  editButtons.forEach((btn) => {
    btn.addEventListener("click", async function () {
      const id = this.getAttribute("data-idReview");

      // Lấy dữ liệu review hiện tại qua AJAX
      const res = await fetch(`/review/get/${id}`);
      const data = await res.json();

      // Đổ dữ liệu vào form
      document.getElementById("edit_review_id").value = data.id;
      document.getElementById("edit_review_text").value = data.review;

      // Chọn đúng số sao
      document.querySelectorAll("#edit_rating input").forEach((radio) => {
        radio.checked = parseInt(radio.value) === parseInt(data.rating);
      });
